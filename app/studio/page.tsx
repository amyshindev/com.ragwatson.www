...
const hubs = [
  {
    ...
    role: "음원을 업로드한 직후, AI가 분석한 음악의 ‘유전자 데이터’를 먼저 확인합니다.",
    detail:
  ...
  },
  {
    ...
    role: "AI 아트웍을 미세 조정하고 생성하는 핵심 툴입니다.",
    detail:
      "중앙의 스포티파이 9:16 미리보기와 우측 커스텀 패널(글리치 강도, 네온 팔레트, 파편화 트리거 비트 등)로 무드를 다듬습니다.",
    Icon: Sparkles,
  },
  {
    href: "/library",
    ...
    detail:
      "4K 영상 다운로드, 지난 프로젝트 재편집, 과거 오디오 분석 히스토리 조회를 제공할 예정입니다.",
    Icon: Library,
  },
] as const
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12 text-zinc-300 md:py-16">
      <header className="mb-10 text-center md:mb-14">
        <p className="text-xs font-bold uppercase tracking-wider text-cyan-500/90">
          Studio
        </p>
        <h1
        ...
        <p className="mx-auto mt-4 max-w-2xl text-sm leading-relaxed text-zinc-500 md:text-base">
          작업 흐름에 맞게 아래에서 분석 대시보드, 커스텀 워크스페이스, 아카이브로
          이동하세요.
        </p>
      </header>
        ...
        {hubs.map(({ href, title, role, detail, Icon }) => (
          <li key={href}>
            <Link href={href} className="group block h-full outline-none">
              <Card className="h-full border-white/10 bg-zinc-950/60 py-0 shadow-lg shadow-black/30 backdrop-blur-xl transition-colors hover:border-cyan-500/35 hover:bg-zinc-950/80">
                <CardHeader className="gap-4 px-6 py-6">
                  <div className="flex items-start justify-between gap-3">
                    <span className="flex h-11 w-11 shrink-0 items-center justify-center border border-zinc-700 bg-black/40 text-cyan-400 transition-colors group-hover:border-cyan-500/40 group-hover:text-cyan-300">
                      <Icon className="h-5 w-5" aria-hidden />
                    </span>
                    <span className="text-xs font-medium text-zinc-600 transition-colors group-hover:text-cyan-500/80">
                      이동 →
                    </span>
                  </div>
                  <div>
